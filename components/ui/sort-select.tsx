    ...
    open && mounted
      ? createPortal(
          <ul
            ...
            style={menuStyle}
            className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_20px_48px_rgba(11,45,96,0.22)]"
          >
  ...
  return (
    ...
    >
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className={`flex h-11 w-full items-center justify-between gap-3 rounded-lg border bg-white px-3.5 text-left text-sm font-medium transition-colors ${
          open
